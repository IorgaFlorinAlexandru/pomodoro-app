...
function Navbar() {
    ...
    const navLinks = NavigationRoutes.map(navLink => 
        <li key={navLink.route} className="mb-2 hover:text-gruvbox-wheat-200 last:mt-auto">
            <NavLink to={navLink.route} className={({ isActive }) => isActive ? 
                ...
                before:block before:content-[" "] before:w-1 before:h-5 before:rounded before:mr-1 before:bg-gruvbox-orange-300` 
                : "flex items-center w-full p-1 pl-2 rounded text-inherit ease-in duration-100 hover:bg-gruvbox-gray-800 before:block before:mr-2"}>
